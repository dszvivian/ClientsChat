pub mod server{
    ...
    pub fn new_server() -> Server {
        ...
        let queues = Arc::new(Mutex::new(new_queue())); // Currently we only need a single queue which contains all the messages from the clients
    ...
    impl Server{
        ...
        pub fn start(self){
            ...
            for stream in conn_listner.incoming(){
                ...
                thread::spawn(move|| {
                    handle_connection(&mut queue_ref,&mut clients,stream);
                });
            }
        }
    ...
    fn extract_request(s:&str) -> (&str,&str){
        //bug -- We have to ensure that there should only be two words in string---> send{client_id} message
